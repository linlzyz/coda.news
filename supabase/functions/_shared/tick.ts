...
import { fixEnglish } from "./fixen.ts";
...
import { finishReels, proposeInstagram } from "./instagram.ts";
...
export async function tick(budgetMs = 120_000, opts: { skipIngest?: boolean } = {}) {
  ...
  if (left() > 20_000) await step("fixen", () => fixEnglish(8));
  ...
  await step("instagram", async () => (await finishReels()) + (await proposeInstagram()));

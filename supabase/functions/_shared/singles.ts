...
Return JSON only: {"summary":"2-3 sentences in English: what happened, who, the key numbers","summary_zh":"same in natural Simplified Chinese","points":["2-4 short English bullet points, each a concrete fact (who, what, where, numbers, dates) that is NOT already in the headline; an outlet's opinion must be attributed (\"${r.source} says ...\"); fewer points for a short article, never pad or guess"],"points_zh":["the same points in Simplified Chinese"]}
Headline: ${r.title}
Article:
${text.slice(0, 4000)}`;
    try {

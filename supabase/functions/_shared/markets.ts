...
const SERIES: [string, string][] = [["S&P 500", "SP500"], ["NASDAQ", "NASDAQCOM"], ["DOW JONES", "DJIA"], ["NIKKEI 225", "NIKKEI225"]];

export async function refreshIndices(force = false): Promise<number> {
  const key = env("FRED_API_KEY"); if (!key) return 0;
  const sql = db();
  if (!force) {
    const [r] = await sql`select max(updated_at) as t from market_series where grp = 'indices'`;
  ...
  let n = 0;
  for (const [i, [name, id]] of SERIES.entries()) {
    try {
      ...
      await sql`insert into market_series (name, grp, sort, value, change, series, digits, as_of, updated_at)
                values (${name}, 'indices', ${i}, ${last.v}, ${((last.v - prev.v) / prev.v) * 100}, ${sql.json(obs.slice(-30).map((o: { v: number }) => o.v))}, 2, ${last.d}, now())
                on conflict (name) do update set value = excluded.value, change = excluded.change, series = excluded.series, as_of = excluded.as_of, updated_at = now(), sort = excluded.sort`;
      n++;
    } catch (e) { log(`fred ${id}:`, (e as Error).message); }
  }
  log(`markets: ${n} indices`);
  return n;
}

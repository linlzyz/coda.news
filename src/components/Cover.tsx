  ...
  if (e.image_url) {
    return (
      <figure className={`relative overflow-hidden bg-[#F4F5F7] ${className}`}>
        <img src={e.image_url} alt={e.title} title={e.image_credit ? `Photo: ${e.image_credit}` : undefined} className="h-full w-full object-cover" style={e.image_focus === "top" ? { objectPosition: "center 18%" } : undefined} loading={priority ? "eager" : "lazy"} fetchPriority={priority ? "high" : "auto"} decoding="async" />

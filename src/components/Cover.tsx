...
export function Cover({ e, className = "", credit = false, iconName, priority = false }: { e: EventRow; className?: string; credit?: boolean; iconName?: string; priority?: boolean }) {
  ...
  if (e.image_url) {
    return (
      <figure className={`group relative overflow-hidden bg-[#F4F5F7] ${className}`}>
        ...
        {e.image_focus === "logo" ? (
          ...
          <>
            <SafeImg src={e.image_url} alt="" aria-hidden className="absolute inset-0 h-full w-full scale-110 object-cover opacity-70 blur-2xl" loading={priority ? "eager" : "lazy"} decoding="async" />
            <SafeImg src={e.image_url} alt={e.title} title={e.image_credit ? `Photo: ${e.image_credit}` : undefined} className="relative h-full w-full object-contain" loading={priority ? "eager" : "lazy"} fetchPriority={priority ? "high" : "auto"} decoding="async" />
          </>
        ) : (
          <SafeImg src={e.image_url} alt={e.title} title={e.image_credit ? `Photo: ${e.image_credit}` : undefined} className="h-full w-full object-cover" loading={priority ? "eager" : "lazy"} fetchPriority={priority ? "high" : "auto"} decoding="async" />
        )}

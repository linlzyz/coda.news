...
export function NewsItem({ e, companies, topics, lang }: { e: EventRow; companies: Map<number, Company>; topics: Map<number, Topic>; lang: Lang }) {
  ...
  return (
    <article className="grid gap-5 border-b border-[#DDE6F1] py-5 sm:grid-cols-[208px_minmax(0,1fr)]">
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-2 text-[11px]">
          ...
          <span className="text-slate-500">{timeAgoL(e.last_article_at, lang)}</span>
          <StatusPill status={e.status} lang={lang} />
        </div>
        <h3 className="mt-2 text-[18px] font-semibold leading-snug tracking-[-0.015em] text-[#0E1A2B]">
          <Link href={`/event/${e.slug}`} className="hover:text-[#1560BD]">{title(e, lang)}</Link>
        </h3>
        {summary(e, lang) && <p className="mt-1.5 line-clamp-2 text-[14px] leading-relaxed text-slate-600">{summary(e, lang)}</p>}
        <div className="mt-3 flex flex-wrap items-center gap-1.5">
          {tags.map((t) => <Link key={t.href} href={t.href} className="rounded-full bg-[#F3F6FA] px-2.5 py-1 text-[12px] font-medium text-slate-700 hover:bg-[#E6EDF6]">{t.label}</Link>)}
          <span className="ml-auto inline-flex items-center gap-2 text-[12px] text-slate-500">

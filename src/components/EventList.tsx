...
import { FeaturedCards, NewsItem, pickFeatured } from "./NewsItem";
...
export async function EventList({ title, intro, events, lang, header }: { title: string; intro?: string; events: EventRow[]; lang: Lang; header?: React.ReactNode }) {
  ...
  const key = pickFeatured(events, 2);
  const keyIds = new Set(key.map((e) => e.id));
  const rest = events.filter((e) => !keyIds.has(e.id));
  return (
    <div className="mx-auto max-w-[960px] px-4 py-10 sm:px-6">
      ...
      {key.length > 0 && <div className="mt-6"><FeaturedCards events={key} lang={lang} heading={t(lang, "keyStories")} /></div>}
      <div className="mt-4">{rest.length ? [...rest].sort((a, b) => Date.parse(b.last_article_at) - Date.parse(a.last_article_at)).map((e, k, arr) => (
        <div key={e.id}>

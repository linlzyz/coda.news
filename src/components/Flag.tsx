...
export function Flag({ code, size = 16 }: { code: string; size?: number }) {
  ...
  const style = { width: size * 1.5, height: size, borderRadius: 3, boxShadow: "0 0 0 1px rgba(14,26,43,.12)" };
  if (!F) return <span className="inline-block bg-[#DDE6F1]" style={style} aria-label={countryName(code)} />;
...
export function Flags({ codes, max = 8, size = 12 }: { codes: string[]; max?: number; size?: number }) {
  ...
  return (
    <span className="inline-flex items-center gap-1" title={codes.map(countryName).join(", ")}>
      ...
      {codes.length > max && <span className="text-xs text-slate-500">+{codes.length - max}</span>}
    </span>
  );
}

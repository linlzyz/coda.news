...
export default async function Legal({ params }: PageProps<"/[lang]/legal/[doc]">) {
  ...
  return (
    <article className="mx-auto max-w-[760px] px-4 py-12 sm:px-6">
      ...
      <p className="mt-2 text-[13px] text-neutral-500">Last updated: {d.updated}</p>
      <div className="mt-8 space-y-8">
        {d.sections.map(([h, body]) => (
          <section key={h}>
            <h2 className="text-[19px] font-semibold tracking-[-0.015em]">{h}</h2>
            <div className="mt-2 space-y-3 text-[15px] leading-relaxed text-neutral-700">{body.split("\n\n").map((p, i) => <p key={i}>{p}</p>)}</div>
          </section>
        ))}
      </div>
    </article>

...
export const DOCS: Record<string, { title: string; updated: string; sections: [string, string][] }> = {
  terms: {
    ...
    sections: [
      ["About these terms", `coda.news ("coda.news", "we", "us") is an online service operated by ${OPERATOR} in Melbourne, Australia, that summarises news and shows how media in different countries report the same events. By using coda.news you agree to these terms. If you do not agree, please do not use the site.`],
      ["What coda.news provides", `coda.news publishes short summaries, extracted facts, timelines and comparisons of coverage. These are generated automatically, largely by artificial intelligence, from publicly available news sources and official releases. We link to every original source.\n\nWe do not republish full articles. Headlines and short excerpts are shown only to identify and link to the original publisher.`],
      ...
      ["Copyright complaints", `If you believe content on coda.news infringes your rights, email ${CONTACT} with the page link, the work concerned and your contact details. We will review and, where appropriate, remove it promptly.`],
  ...
  privacy: {
    ...
    sections: [
      ...
      ["Who processes it", `We use trusted service providers to run coda.news: Vercel (website hosting), Supabase (database), and, when newsletters are sent, an email delivery provider. These providers may store data outside Australia, including in Singapore and the United States. We take reasonable steps to ensure they protect it.`],
  ...
  cookies: {
    ...
    sections: [
      ...
      ["Third parties", `Images are loaded from the servers of the image libraries we use (for example Pexels, Unsplash or Wikimedia). Those servers receive standard technical information such as your IP address when your browser loads an image. Our hosting provider may use strictly necessary cookies to protect the site from abuse.`],

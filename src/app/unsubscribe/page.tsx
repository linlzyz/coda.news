...
export default async function Page({ searchParams }: PageProps<"/unsubscribe">) {
  ...
  const [ok, l] = await Promise.all([unsubscribe(token), getLang()]);
  const zh = l === "zh";
  return (
    <div className="mx-auto max-w-[560px] px-4 py-20 text-center sm:px-6">
      ...
      <p className="mt-3 text-neutral-600">{ok ? (zh ? "你不会再收到 Coda 每日简报。随时欢迎回来。" : "You won't receive The Daily Coda any more. You're welcome back any time.") : (zh ? "你可能已经退订过了。" : "You may already be unsubscribed.")}</p>

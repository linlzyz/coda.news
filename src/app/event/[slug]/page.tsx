...
export default async function EventPage({ params }: PageProps<"/event/[slug]">) {
  ...
  return (
    <div className="mx-auto max-w-[1240px] px-4 py-8 sm:px-6">
      ...
      <nav className="mb-6 text-[13px] text-neutral-500"><Link href="/" className="hover:text-[#111111]">{t(l, "home")}</Link><span className="mx-1.5">/</span><Link href={`/${e.category}`} className="hover:text-[#111111]">{t(l, e.category)}</Link></nav>
      ...
      <div className="grid gap-10 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="min-w-0 space-y-8">
          <header className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 text-[13px]">
              ...
              {shownCos.map((c) => <Link key={c.id} href={`/company/${c.slug}`} className="rounded-full bg-[#FFF3EC] px-3 py-1 font-medium text-[#C2410C] hover:bg-[#FFE6D8]">{c.name}</Link>)}
            </div>
          </header>
          ...
          {agreed.length > 0 && (
            <section className="border-t-2 border-[#111111] pt-3">
              <h2 className="text-[20px] font-semibold tracking-[-0.02em]">{t(l, "agreed")}</h2>
          ...
          {perspectives.length >= 2 && (
            <section className="bg-[#111111] p-6 text-white">
                  ...
                  {(["positive", "neutral", "negative"] as const).map((tone, i) => {
                    ...
                    return (
                      <div key={tone} className={`flex flex-col ${i === 0 ? "items-start" : i === 1 ? "items-center" : "items-end"}`}>
                        <span className={`h-[22px] w-[22px] rounded-full border-4 border-[#111111] ${group.length ? "bg-[#C2410C]" : "bg-neutral-600"}`} />
          ...
          <section className="space-y-4">
            <h2 className="border-t-2 border-[#111111] pt-3 text-[20px] font-semibold tracking-[-0.02em]">{t(l, "howEach")}</h2>
            {perspectives.length === 0 && <p className="text-neutral-500">{t(l, "oneCountry")}</p>}
            <div className="grid gap-4 md:grid-cols-2">
              {perspectives.map((p) => {
                const v = perspL(p, l);
                return (
                  <article key={p.country} className="flex flex-col gap-3 border-t border-[#111111] pt-4">
                    ...
                    <div className="mt-auto border-t border-[#EFEFEF] pt-3 text-[12px] text-neutral-500">{n(p.article_count, "article", "articles")} · {[...new Set((byCountry.get(p.country) ?? []).map((a) => a.sources.name))].join(", ")}</div>
                  </article>
                );
              })}
            </div>
          ...
          <section className="space-y-3">
            <h2 className="border-t-2 border-[#111111] pt-3 text-[20px] font-semibold tracking-[-0.02em]">{t(l, "sourcesH")}</h2>
            <p className="text-[12px] text-neutral-500">{t(l, "disclaimer")}</p>
            {[...byCountry].map(([c, list]) => (
              <div key={c} className="border-b border-[#E6E6E6] pb-4">
        ...
        <aside className="space-y-5">
          {timeline.length > 0 && (
            <section className="border-t-2 border-[#111111] pt-3">
              <h2 className="text-[20px] font-semibold tracking-[-0.02em]">{t(l, "timeline")}</h2>
              <ol className="mt-4">
                {timeline.map((f, i) => (
                  <li key={f.id} className="grid grid-cols-[12px_minmax(0,1fr)] gap-3">
                    <div className="flex flex-col items-center"><span className={`mt-1.5 h-2.5 w-2.5 rounded-full ${i === 0 ? "bg-[#C2410C]" : "bg-neutral-300"}`} /><span className="w-px flex-1 bg-[#E6E6E6]" /></div>
                    <div className="pb-4"><div className="text-[12px] text-neutral-500">{fmtDate(f.occurred_at)}{f.source_ids.length > 1 ? ` · ${n(f.source_ids.length, "source", "sources")}` : ""}</div><div className="text-[14px] leading-snug">{f.content.text || `${f.content.subject} ${f.content.predicate} ${f.content.object}`}</div></div>
                  </li>
                ))}
              </ol>
            </section>
          ...
          {related.length > 0 && (
            <section className="border-t-2 border-[#111111] pt-3">
              <h2 className="text-[20px] font-semibold tracking-[-0.02em]">{t(l, "moreOn")} {shownCos[0].name}</h2>
              <div className="mt-2 divide-y divide-[#EFEFEF]">
                {related.map((r) => (
...
function Chip({ children, strong }: { children: React.ReactNode; strong?: boolean }) {
  return <span className={`rounded-full px-3 py-1 ${strong ? "bg-[#111111] text-white" : "bg-[#F5F5F4] text-neutral-700"}`}>{children}</span>;
}
